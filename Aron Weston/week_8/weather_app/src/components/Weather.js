    ...
    if (input.coords) {
      try {
        const res = await axios.get(
          `https://api.openweathermap.org/data/2.5/weather?lat=${input.coords.latitude}&lon=${input.coords.longitude}&appid=${this.key}&units=metric`
        );
      ...
      try {
        const weather = await axios.get(
          `https://api.openweathermap.org/data/2.5/weather?q=${input}&appid=${this.key}&units=metric`
        );
      } catch (error) {
        console.error(error);
      }
