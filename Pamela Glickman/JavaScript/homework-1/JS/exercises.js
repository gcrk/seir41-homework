...
const calculations = function(num) {
  ...
  const squaredNum = squareNumber(halfedNum);
  const area = areaOfCircle(squaredNum);
  const percent = Math.round(percentOf(area, squaredNum));
  return percent;
}
...
const DrEvil = function(amount) {
    if (amount === 1000000) {
      const message = `${ amount} + dollars (pinky)`;
      return message;
    } else {
      const message = `${ amount} + dollars`;
      return message;
    }
}
//
...
const mixUp = function (string1, string2) {
  const result = string2.slice(0,2) + string1.slice(2) + " " + string1.slice(0,2) + string2.slice(2);
  console.log(result);
  return;
}
//
