...
function recursiveFlood(x, y, targetValue, newValue){
   // Stop the flood when it reaches the edge of the board or a square that isn't the target colour
   if (x < 0 || x > 9 || y < 0 || y > 9 || gameBoard[y][x] !== targetValue) {
     return;
   }
   // Update the game board square with it's new color value
   gameBoard[y][x] = newValue;
   // Render the square with it's new color
   renderSquare(x, y);
   // Call the recursiveFlood function for adjacent squares
